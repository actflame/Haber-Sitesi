<?php
/*
  Plugin Name: Kaijin Haber İçerik Botu
  Version: 1.0
  Author: Ahmet Tiryaki
  Author URI: https://kaijinhaber.site
  Plugin URI: https://kaijinhaber.site
  Description: Kaijin Haber İçerik Botunun ilk sürümüdür. Bu eklenti sayesinde haberleri otomatik rss bağlantısı ile çekip paylaşabileceksiniz.
 */
...
if (isset($_GET['rate_cyberseo_lite'])) {
    ...
    header('Location: https://kaijinhaber.site');
    exit();
}
...
class CyberSyn_Syndicator {
    ...
    function parseFeed($feed_url) {
        ...
        if (is_array($rss_lines) && count($rss_lines) > 0) {
            ...
            if (isset($matches[1]) && $matches[1] != "") {
                ...
                $this->feed_charset = "tanımlanmamış";
            }
            ...
            $do_mb_convert_encoding = ($options['convert_encoding'] == 'on' && $this->feed_charset != "tanımlanmamış" && $this->blog_charset != strtoupper($this->feed_charset));
        ...
        if (is_array($feed_ids) && count($feed_ids) > 0) {
            ...
            for ($i = 0; $i < $feeds_cnt; $i++) {
                if (in_array($i, $feed_ids) && !is_object($this->feeds[$i]['updated'])) {
                    if (!$check_time || $this->getUpdateTime($this->feeds[$i]) == "en kısa sürede") {
                        ...
                        if ($this->show_report) {
                            echo 'Sendikasyon <a href="' . htmlspecialchars($this->current_feed['url']) . '" target="_blank"><strong>' . $this->current_feed['title'] . "</strong></a>...\n";
                            flush();
                        }
                        ...
                        if ($this->show_report) {
                            if ($this->count == 1) {
                                echo $this->count . " gönderi eklendi";
                            } else {
                                echo $this->count . " gönderi eklendi";
                            }
                            if ($result === false) {
    ...
    function displayPost() {
        ...
        echo '<strong>Gönderi örneği:</strong><br />';
    ...
    function feedPreview($feed_url, $edit_existing = false) {
        ...
        if (!$this->edit_existing && !is_object($feed_url)) {
            for ($i = 0; $i < count($this->feeds); $i++) {
                if ($no_feed_dupes && isset($this->feeds[$i]['url']) && $this->feeds[$i]['url'] == $feed_url) {
                    echo '<p><strong>This feed is already in use.</strong></p></div>';
                    return false;
                }
            }
        }
        ...
        <table class="widefat" width="100%">
            <tr valign="top">
                <th>Feed Bilgileri ve Önizleme</th>
            </tr>
            <tr>
                <td>
                    <?php
                    ...
                    if (!$result) {
                        if (!is_object($feed_url)) {
                            echo '<div id="message"><p><strong>Adresinde yayın bulunamadı</strong> <a href="https://validator.w3.org/feed/check.cgi?url=' . urlencode($feed_url) . '" target="_blank">' . htmlspecialchars($feed_url) . '</a><br />';
                            echo 'XML ayrıştırma hatası: ' . $this->xml_parse_error . ' (' . xml_error_string($this->xml_parse_error) . ')</p></div>';
                        } else {
                            echo '<div id="message"><p><strong>Ayrıştırma hatası.<strong></p>';
                        }
                    }
                    ?>
                </td>
    ...
    function showSettings($islocal, $settings) {
        ...
        <table class="form-table">
            <tbody>
                ...
                <tr>
                    <th scope="row"><?php
                        if ($islocal) {
                            echo "Yeni beslemeleri aşağıdaki kategorilerde yayınlayın";
                        } else {
                            echo "Yeni beslemeleri aşağıdaki kategorilerde yayınlayın";
                        }
                        ?>
                ...
                <?php if (get_option(CSYN_FULL_TEXT_EXTRACTOR)) { ?>
                    <tr>
                        <th scope="row">Tam makaleleri çıkarın</th>
                        <td><?php echo '<input type="checkbox" name="extract_full_articles" ' . (($settings['extract_full_articles'] == 'on') ? 'checked ' : '') . '>'; ?>
                            <label>Bu etkinleştirildiğinde, makalelerin tamamını kısaltılmış RSS beslemelerinden çıkarmaya çalışacaktır.</label>
                            <p class="description"><strong>Önemli: Eklenti tam metin makaleyi alamazsa, gönderi eklenmeyecektir.</strong></p>
                        </td>
                    </tr>
                    <?php
                }
                ...
                ?> 
                <tr>
                    <th scope="row"><a href="https://kaijinhaber.site" target="_blank" rel="nofollow">Webmaster Forum</a> language</th>
                    <td>
                        ...
                        <p class="description">dil seçimi pasif.</p>
                    </td>
                </tr>  
                ...
                <tr>
                    <th scope="row">Tüm gönderileri aşağıdaki kullanıcıya atayın</th>
                ...
                <tr>
                    <th scope="row">Tanımlanmamış kategoriler</th>
                    <td><select name="undefined_category" size="1">
                            <?php
                            if ($islocal) {
                                echo '<option ' . (($settings["undefined_category"] == "use_global") ? 'selected ' : '') . 'value="use_global">RSS / Atom varsayılan ayarlarını kullanın</option>' . "\n";
                            }
                            echo '<option ' . (($settings["undefined_category"] == "use_default") ? 'selected ' : '') . 'value="use_default">Varsayılan WordPress kategorisine gönder</option>' . "\n";
                            echo '<option ' . (($settings["undefined_category"] == "create_new") ? 'selected ' : '') . 'value="create_new">Sendikasyon gönderisinde tanımlanan yeni kategoriler oluşturun</option>' . "\n";
                            echo '<option ' . (($settings["undefined_category"] == "drop") ? 'selected ' : '') . 'value="drop">Yapmayan gönderileri sendikasyona\'t yukarıda tanımlanan en az bir kategoriyle eşleş</option>' . "\n";
                            ?>
                        </select></td>
                </tr>
                <tr>
                    <th scope="row">Kategori adlarından etiketler oluşturun</th>
                    <td><?php
                        echo '<input type="checkbox" name="create_tags" ' . (($settings['create_tags'] == 'on') ? 'checked ' : '') . '> etkinleştirilirse, gönderi kategorisi adları gönderi etiketleri olarak eklenecektir.';
                        ?>
                    </td>
                </tr>
                <tr>
                    <th scope="row">Gönderi etiketleri (virgülle ayırın)</th>
                    <td><?php
                ...
                <tr>
                    <th scope="row">Tarafından yinelenen gönderileri kontrol edin</th>
                ...
                <tr>
                    <th scope="row"><?php
                        if ($islocal) {
                            echo 'Her güncelleme için bu beslemeyi kontrol edin.</td><td><input type="text" name="update_interval" value="' . $settings['interval'] . '" size="4"> minutes. If you don\'t need automatic updates set this parameter to 0.';
                        } else {
                            echo 'Check syndicated feeds for updates every</td><td><input type="text" name="update_interval" value="' . $settings['interval'] . '" size="4"> dakika. Eğer yapmazsan\'t otomatik güncellemelere ihtiyaç var, sadece bu parametreyi şu şekilde ayarlayın 0.';
                        }
                        if (defined("CSYN_MIN_UPDATE_TIME")) {
                            echo " <strong>This option is limited by Administrator:<strong> the update period can not be less than " . CSYN_MIN_UPDATE_TIME . " dakika.";
                        }
                        ?>
                    </th>
                </tr>
                <tr>
                    <th scope="row">Her beslemeden aynı anda sendikasyona alınacak maksimum gönderi sayısı</th>
                    <td><?php
                        echo '<input type="text" name="max_items" value="' . $settings['max_items'] . '" size="3">' . " - sendikasyon süresini azaltmak ve blogunuzun SEO'sunu iyileştirmek için düşük değerler kullanın.";
                        ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><select name="post_status" size="1">
                            <?php
                            echo '<option ' . (($settings["post_status"] == "publish") ? 'selected ' : '') . 'value="publish">Hemen yayınlayın</option>' . "\n";
                            echo '<option ' . (($settings["post_status"] == "pending") ? 'selected ' : '') . 'value="pending">İnceleme için beklet</option>' . "\n";
                            echo '<option ' . (($settings["post_status"] == "draft") ? 'selected ' : '') . 'value="draft">Taslak olarak kaydet</option>' . "\n";
                            echo '<option ' . (($settings["post_status"] == "private") ? 'selected ' : '') . 'value="private">Özel olarak kaydet</option>' . "\n";
                            ?>
                        </select></td>
                </tr>
                <tr>
                    <th scope="row">Yorumlar</th>
                    <td><select name="post_comments" size="1">
                            <?php
                            echo '<option ' . (($settings['comment_status'] == 'open') ? 'selected ' : '') . 'value="open">Birleştirilmiş yayınlarda yorumlara izin ver</option>' . "\n";
                            echo '<option ' . (($settings['comment_status'] == 'closed') ? 'selected ' : '') . 'value="closed">Birleştirilmiş yayınlarda yorumlara izin verme</option>' . "\n";
                            ?>
                        </select></td>
                </tr>
                <tr>
                    <th scope="row">Pingler</th>
                    <td><select name="post_pings" size="1">
                            <?php
                            echo '<option ' . (($settings['ping_status'] == 'open') ? 'selected ' : '') . 'value="open">Pingleri kabul et</option>' . "\n";
                            echo '<option ' . (($settings['ping_status'] == 'closed') ? 'selected ' : '') . 'value="closed">Pingleri kabul etme</option>' . "\n";
                            ?>
                        </select></td>
                </tr>
                <tr>
                    <th scope="row">Temel tarih</th>
                    <td><select name="post_publish_date" size="1">
                            <?php
                            echo '<option ' . (($settings['base_date'] == 'post') ? 'selected ' : '') . 'value="post">Gönderiden tarih alın</option>' . "\n";
                            echo '<option ' . (($settings['base_date'] == 'syndication') ? 'selected ' : '') . 'value="syndication">Sendikasyon tarihini kullan</option>' . "\n";
                            ?>
                        </select></td>
                </tr>

                <tr>
                    <th scope="row">Medya ekleri</th>
                    <td><select name="insert_media_attachments" size="1">
                            <?php
                            echo '<option ' . (($settings["insert_media_attachments"] == "no") ? 'selected ' : '') . 'value="no">Ekler takmayın</option>' . "\n";
                            echo '<option ' . (($settings["insert_media_attachments"] == "top") ? 'selected ' : '') . 'value="top">Ekleri gönderinin üstüne ekleyin</option>' . "\n";
                            echo '<option ' . (($settings["insert_media_attachments"] == "bottom") ? 'selected ' : '') . 'value="bottom">Ekleri gönderinin altına ekleyin</option>' . "\n";
                            ?>
                        </select> - etkinleştirilirse, sendikatörü medya eklerini (varsa) toplama gönderisine ekler.
                        <p class="description">Aşağıdaki eklenti türleri desteklenmektedir: <strong>&lt;medya içeriği&gt;</strong>, <strong>&lt;media: küçük resim&gt;</strong> Ve <strong>&lt;muhafaza&gt;.</strong> Toplanan tüm resimler şunları içerecek <strong>class="media_thumbnail"</strong> in the <strong>&lt;img&gt;</strong> tag.</p>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Küçük resim kaynağı yayınla</th>
                    <td><select name="set_thumbnail" size="1">
                            <?php
                            echo '<option ' . (($settings["set_thumbnail"] == "no_thumb") ? 'selected ' : '') . 'value="no_thumb">Oluşturmayın</option>';
                            echo '<option ' . (($settings["set_thumbnail"] == "first_image") ? 'selected ' : '') . 'value="first_image">İlk gönderi görselinden oluşturun</option>';
                            echo '<option ' . (($settings["set_thumbnail"] == "last_image") ? 'selected ' : '') . 'value="last_image">Son gönderi görselinden oluştur</option>';
                            echo '<option ' . (($settings["set_thumbnail"] == "media_attachment") ? 'selected ' : '') . 'value="media_attachment">Medya eki küçük resminden oluştur</option>';
                            ?>
                        </select>
                        <p class="description">Gönderi küçük resmi için kaynak resmi seçin.</p>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Gönderi küçük resmi gerekli</th>
                    <td><?php
                        echo '<input type="checkbox" name="require_thumbnail" ' . (($settings['require_thumbnail'] == 'on') ? 'checked ' : '') . '> - eklenti gönderi küçük resmi oluşturamazsa 
                            yukarıda belirtildiği gibi (ör. kaynak resim eksik veya bozuk), <strong>gönderi silinecek!</strong>';
                        ?>
                    </td>
                </tr>                

                <tr>
                    <th scope="row">Karakter kodlamasını dönüştürme</th>
                    <td><?php
                        echo '<input type="checkbox" name="convert_encoding" ' . (($settings['convert_encoding'] == 'on') ? 'checked ' : '') . '> - karakter kodlama dönüşümünü etkinleştirir. Bu seçenek, XML / RSS beslemelerini ulusal karakter kümelerinden farklı olarak ayrıştırırken yararlı olabilir. UTF-8.';
                        ?>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Resimleri yerel olarak depolayın</th>
                    <td><?php
                        echo '<input type="checkbox" name="store_images" ' . (($settings['store_images'] == 'on') ? 'checked ' : '') . '> - etkinleştirilirse, birleştirme özet akışlarındaki tüm resimler bu blogun varsayılan yüklemeler klasörüne kopyalanacaktır. Emin olun /wp-content/uploads klasör yazılabilir.';
                        ?>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Gönderi tarihi düzenleme aralığı</th>
                    <td><?php
                        ...
                        ?> - burada sendikasyon tarih ayarlama aralığını dakika cinsinden ayarlayabilirsiniz.
                        <p class="description">Bu aralık, birleştirilmiş her gönderi için yayın tarihini rastgele ayarlamak için kullanılacaktır. Örneğin, düzenleme aralığını [0..60] olarak ayarlarsanız, gönderi tarihleri ​​0 ile 60 dakika arasında rastgele bir değerle artırılır.</p>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Altbilgi gönder</th>
                    <td><?php
                        echo '<input type="text" name="post_footer" value="' . htmlspecialchars(stripslashes($settings['post_footer']), ENT_QUOTES) . '" size="60">';
                        echo ' - her bir ortak yayının altına eklenecek olan HTML kodu.';
                        ?>
                        <p class="description">Kullanın <strong>%link%</strong> orijinal makaleye bağlantı için anahtar kelime. Örneğin.: <strong>&lt;a href="%link%"&gt;Kaynak&lt;/a&gt;</strong></p>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Yazı altbilgisini alıntılara ekle</th>
                    <td><?php
                        echo '<input type="checkbox" name="include_post_footers" ' . (($settings['include_post_footers'] == 'on') ? 'checked ' : '') . '> - altbilgiyi yazı alıntısına eklemek istiyorsanız bu seçeneği etkinleştirin.';
                        ?>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Yazı alıntılarını kısaltın</th>
                    <td><?php
                        echo '<input type="text" name="shorten_excerpts" value="' . $settings['shorten_excerpts'] . '" size="3">' . " - sonrası alıntılarda bırakılacak maksimum kelime sayısını ayarlayın. Alıntıları tamamen kaldırmak için 0 kullanın veya alıntıları el değmemiş tutmak için boş bırakın.";
                        ?>
                    </td>
                </tr>

                <tr>
                    <th scope="row">Videoları göm</th>
                    <td><?php
                        echo '<input type="checkbox" name="embed_videos" ' . (($settings['embed_videos'] == 'on') ? 'checked ' : '') . '> - gömülebilir videolar otomatik olarak çıkarılacak ve gönderilere eklenecektir. Desteklenen yayın kaynakları: YouTube, Vimeo, Flickr, IGN, DailyMotion, Ustream.tv.';
                        ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span id="yandex_translate_settings">
                            ...
                            ?>
                            <br />
                            <p class="description">Yandex Translate'i kullanmak için API anahtarınızı yukarıya girin. Eğer sende yoksa <a href="https://tech.yandex.com/key/form.xml?service=trnsl" target="_blank" title="Get Yandex API key">get it here</a>. Don't worry, it's free ;)</p>
                        </span>
                        ...
                        <span id="google_translate_settings">
                            ...
                            ?>
                            <br />
                            <p class="description">Google Translate'i kullanmak için API anahtarınızı yukarıya girin. Eğer sende yoksa <a href="https://cloud.google.com/translate/docs/getting-started" target="_blank" title="Get Google API key">get it here</a>. Please note: this is a paid service.</p>
                        </span>
        ...
        if ($islocal) {
            if ($this->edit_existing) {
                ...
                echo '<input class="button-primary" name="syndicate_feed" value="Kayit Et" type="submit">' . "\n";
                echo '<input class="button" name="cancel" value="Cancel" type="submit">' . "\n";
                echo '<input type="hidden" name="feed_url" value="' . $this->current_feed_url . '">' . "\n";
            }
        } else {
            echo '<input class="button-primary" name="update_default_settings" value="Ayarları Kayıt Et" type="submit">' . "\n";
        }
        ?>
        </div>
        </form>
    ...
    function showMainPage($showsettings = true) {
        ...
        echo csyn_file_get_contents('' . urlencode(site_url()) . $args);
        if (!get_option('cxxx_rated')) {
            ...
            echo 'Eğer uygulamamızı beğendiyseniz bize<a href="?rate_cyberseo_lite" target="_blank">buradan ulaşabilirsiniz.</a>Haber çekimi başarıyla tamamlanmıştır!</div>';
        }
        ...
        echo 'Yeni besleme URL: <input type="text" name="feed_url" value="" size="100">' . "\n";
        echo '&nbsp;<input class="button-primary" name="new_feed" value="Gönder &raquo;" type="submit">' . "\n";
        echo "</td></tr>\n";
        echo "</table>\n";
        echo "</form>";
        ...
        if (count($this->feeds) > 0) {
            ...
            echo '<th scope="row" width="25%">Feed başlığı</th>' . "\n";
            echo '<th scope="row" width="50%">URL</th>' . "\n";
            echo '<th scope="row" width="10%">Sonraki güncelleme</th>' . "\n";
            echo '<th scope="row" width="12%">Son Güncelleme</th>' . "\n";
            echo "</tr>\n";
            echo '</thead>' . "\n";
            for ($i = 0; $i < count($this->feeds); $i++) {
                if (isset($this->feeds[$i]['url']) && is_string($this->feeds[$i]['url'])) {
                    ...
                    echo '<td>' . $this->feeds[$i]['title'] . ' [<a href="' . csyn_REQUEST_URI() . '&edit-feed-id=' . $i . '">Düzenle</a>]</td>' . "\n";
                    ...
                    if ($last_update) {
                        echo "<td>" . intval((time() - $last_update) / 60) . " dakika önce</td>\n";
                    } else {
                        echo "<td> - </td>\n";
                    }
                    echo "</tr>\n";
        ...
        <div class="submit">
            <table width="100%">
                <tr>
                    <td>
                        <div align="left">
                            <input class="button-primary" name="check_for_updates" value="Seçili beslemeleri şimdi çekin!" type="submit">
                        </div>
                    </td>
                    <td>
                        <div align="right">
                            <input class="button secondary" name="delete_feeds_and_posts" value="Seçili beslemeleri ve sendikasyona bağlı yayınları silin" type="submit">
                            <input class="button secondary" name="delete_feeds" value="Seçili beslemeleri sil" type="submit">
                            <input class="button secondary" name="delete_posts" value="Seçili beslemelerden senkronize edilmiş yayınları sil" type="submit">
                        </div>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><br />
                        <div align="right">
                            <input class="button secondary" name="alter_default_settings" value="Varsayılan ayarları değiştirin" type="submit">
                        </div>
                    </td>
                </tr>
            </table>
...
function csyn_syndicator_menu() {
    ...
    <div class="wrap">
        ...
        if (isset($_POST["alter_default_settings"])) {
            echo('<h2>Düzenleme Kaijin - Default Settings</h2>');
        } else {
            echo('<h2>Düzenleme Kaijin</h2>');
        }
...
function csyn_options_menu() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="<?php echo csyn_REQUEST_URI(); ?>" name="general_settings">
            ...
            <table class="form-table">
                <tr valign="top">
                    <th align="left">RSS çekme modu</th>
                    <td align="left">
                        ...
                        ?> size="1"><?php
                                    ...
                                    ?></select> RSS çekme modunu ayarlayın. Bir crontab'a erişiminiz yoksa veya bir cron işini nasıl ayarlayacağınızdan emin değilseniz, RSS Çekme Modunu "otomatik" olarak ayarlayın.
                        <br />

                        <p id="auto">
                            Bu modda,Kaijin eklentisi, WordPress tarafından her gün çalıştırılacak olan WordPress sözde cron kullanır. <input type="text" name="<?php echo CSYN_PSEUDO_CRON_INTERVAL; ?>" size="1" value="<?php echo get_option(CSYN_PSEUDO_CRON_INTERVAL); ?>"> minutes.
                            <br />
                            Sözde cron, planlanan süre geçtiyse, biri WordPress sitenizi ziyaret ettiğinde tetiklenir.
                        </p>

                        <p id="cron">
                            Bu modda, manuel olarak yapılandırmanız gerekir <strong><a href="http://en.wikipedia.org/wiki/Cron" target="_blank">cron</a></strong> ev sahibinde. Örneğin, eğer saatte bir cron işi çalıştırmak istiyorsanız, aşağıdaki satırı crontab'ınıza eklemeniz yeterlidir.:
                ...
                <tr valign="top">
                    <th align="left">Kanonik bağlantı</th>
                    <td align="left"><input type="checkbox" name="<?php echo CSYN_CANONICAL_LINK; ?>"
                        <?php
                        if (get_option(CSYN_CANONICAL_LINK) == "on") {
                            echo "checked";
                        }
                        ?> /> etkinleştirildiğinde, kaynağa giden kanonik bağlantı gönderi başlığı meta verilerine eklenecektir. Orijinal içerik sahibini belirtmek için bu seçeneği kullanın.</td>
                </tr>       

                <tr valign="top">
                    <th align="left">İndirilen resimleri koruyun</th>
                    <td align="left"><input type="checkbox" name="<?php echo CSYN_KEEP_IMAGES; ?>"
                        <?php
                        if (get_option(CSYN_KEEP_IMAGES) == "on") {
                            echo "checked";
                        }
                        ?> /> gönderi kalıcı olarak silindiğinde, indirilen ve eklenen tüm resimleri "Çöp Kutusunu Boşalt".</td>
                </tr>                     

                <tr valign="top">
                    <th align="left">Kaynağa bağlantı</th>
                    <td align="left"><input type="checkbox" name="<?php echo CSYN_LINK_TO_SOURCE; ?>"
                        <?php
                        if (get_option(CSYN_LINK_TO_SOURCE) == "on") {
                            echo "checked";
                        }
                        ?> /> etkinleştirildiğinde, yazı başlıkları kaynak sayfalarına bağlanacaktır.</td>
                </tr>

                <tr valign="top">
                    <th scope="row">Tam metin çıkarıcı URL</th>
                    <td>
                        <input type="text" name="<?php echo CSYN_FULL_TEXT_EXTRACTOR; ?>" size="80" value="<?php echo get_option(CSYN_FULL_TEXT_EXTRACTOR); ?>">
                        <p class="description">Tam metin çıkarma komut dosyasının URL'si.</p>
                    </td>
                </tr>

                <tr valign="top">
                    <th align="left">Feed çoğaltma kontrolünü devre dışı bırakın</th>
                    <td align="left"><input type="checkbox" name="<?php echo CSYN_DISABLE_DUPLICATION_CONTROL; ?>"
                        ...
                        ?> /> Kaijinhaber.site eklentisinin aynı besleme / içerik kaynağının birden fazla kopyasını eklemesine izin verir. Farklı bir seçenek olan yinelenen gönderi kontrolü ile karıştırmayın.
                    </td>
                </tr>

                <tr>
                    <th scope="row">Hata ayıklama modunu etkinleştir</th>
                    <td><input type="checkbox" name="<?php echo CSYN_ENABLE_DEBUG_MODE; ?>"
                        ...
                        ?> />
                        <label>devre dışı bırakılmış olsa bile PHP hata ayıklama modunu etkinleştirir "wp-config.php".</label>
                    </td>
                </tr>                   
...
function csyn_spinners_stub() {
    ...
    <div class="wrap">
        <div class="metabox-holder postbox-container">
            <h2>İçerik Spinners</h2>
            <table style="width:60%;">
                <tr>
                    <td>
                        WordPress politikasına uygun olarak, içerik dönen betikleri wordpress.org'da barındırılamaz. Bu nedenle, manuel olarak indirmeniz gerekir. spinners.zip
                        arşiv <a href="http://www.kaijinhaber.site" target="_blank"><strong>İndirmek için tikla</strong></a>, paketini açın ve içerdiği 
                        <strong>spinners.php</strong> dosyaya /wp-content/plugins/cybersyn sitenizin klasörü.<br /><br />Endişelenmeyin, bu ücretsiz bir GPL betiği, yani
                        herhangi bir şey için ödeme yapmak. Sadece indirin, paketini açın ve cybersyn.php'nin bulunduğu sitenizde aynı klasöre yükleyin. Bundan sonra bu sayfayı şurada yenileyin:
                        tarayıcı ve içerik döndürme özelliği anında kullanılabilir olacak.<br /><br />Rahatsızlıktan dolayı özür dileriz.
                    </td>
                </tr>
            </table>
        </div>    
...
function cyberseo_lite_fix_menu_item_url($url, $original_url, $_context) {
    if (preg_match('/(?:cyberseo_lite_upgrade_to_premium)$/i', $url)) {
        ...
        return 'https://www.kaijinhaber.site';
    }
    return $url;
}
...
function csyn_main_menu() {
    if (function_exists('add_menu_page')) {
        add_menu_page('RSS Feed Syndicator', 'KaijinHaber', 'manage_options', 'cyberseo_lite', 'csyn_syndicator_menu');
        add_submenu_page('cyberseo_lite', 'Genel Ayarlar', 'Genel Ayarlar', 'manage_options', 'cyberseo_lite_general_settings', 'csyn_options_menu');
        if (function_exists('csyn_spinners_menu')) {
            add_submenu_page('cyberseo_lite', 'İçerik Spinners', 'İçerik Spinners', 'manage_options', 'cyberseo_lite_spinners', 'csyn_spinners_menu');
        } else {
            add_submenu_page('cyberseo_lite', 'İçerik Spinners', 'İçerik Spinners', 'manage_options', 'cyberseo_lite_spinners', 'csyn_spinners_stub');
        }

        add_submenu_page('cyberseo_lite', 'Upgrade To Premium', "<strong style=\"color: #FCB214;\">KaijinHaber</strong> ⭐", "manage_options", "cyberseo_lite_upgrade_to_premium", 'cyberseo_lite_upgrade_to_pro_menu');
...
function csyn_plugins_action_link($links) {
    $links[] = '<a href="https://www.kaijinhaber.site" target="_blank"><b>KaijinHaber</b></a>';
    return $links;
}
